...
class ReporteBloc extends Bloc<ReporteEvent, ReporteState> {
  ...
  Future<void> _onEnviarReporte(
    ...
    try {
      ...
      if (exito) {
        ...
        emit(
          const ReporteError(errorMessage: ReporteConstantes.errorCrearReporte),
        );
      }
    } on ApiException catch (e) {
      // Si es una ApiException, emitir un ReporteError con el mensaje y código de estado
      emit(ReporteError(errorMessage: e.message, statusCode: e.statusCode));
    } catch (e) {
  ...
  Future<void> _cargarEstadisticas(
    ...
    final estadisticas = await _reporteRepository
        .obtenerEstadisticasReportesPorNoticia(noticiaId);
    emit(
