...
class _ReporteDialogContentState extends State<_ReporteDialogContent> {
  ...
  void _cargarEstadisticasReportes() {
    ...
    context.read<ReporteBloc>().add(
      CargarEstadisticasReporte(noticiaId: widget.noticiaId),
    );
  }
    ...
    return BlocListener<ReporteBloc, ReporteState>(
      listener: (context, state) {
        if (state is ReporteSuccess) {
          ...
          SnackBarHelper.mostrarError(context, mensaje: state.errorMessage);
          setState(() {
            _reporteEnviando = false;
          });
        } else if (state is ReporteEstadisticasLoaded &&
            state.noticiaId == widget.noticiaId) {
              ...
              Row(
                ...
                children: [
                  _buildMotivoButton(
                    ...
                    label: 'Inapropiada',
                    iconNumber:
                        '${_estadisticasReportes['NoticiaInapropiada']}',
                    isLoading:
                        _reporteEnviando &&
                        _ultimoMotivoReportado ==
                  ...
                  _buildMotivoButton(
                    ...
                    isLoading:
                        _reporteEnviando &&
                        _ultimoMotivoReportado == MotivoReporte.otro,
                    smallSize: true, // Indicador para tamaño reducido
                  ),
                ],
              ),
  ...
  Widget _buildMotivoButton({
    ...
    return Column(
      children: [
        InkWell(
          onTap:
              _reporteEnviando ? null : () => _enviarReporte(context, motivo),
          child: Container(
            width: buttonSize,
            height: buttonSize,
  ...
  void _enviarReporte(BuildContext context, MotivoReporte motivo) {
    ...
    context.read<ReporteBloc>().add(
      EnviarReporte(noticiaId: widget.noticiaId, motivo: motivo),
    );
  }
}
