...
class TareasService {
  ...
  final AssistantRepository _assistantRepository =
      AssistantRepository(); // Instancia del nuevo repositorio
  ...
  Future<List<Task>> getMoreTasksWithSteps({
    required int inicio,
    int limite = 4,
  }) async {
    // Simula un retraso para imitar una llamada a una API
    ...
    final tareas =
        _taskRepository.getTasks().skip(inicio).take(limite).toList();

    // Itera sobre cada tarea y reemplaza la fecha límite
    final tareasConFechaActualizada =
        tareas.map((tarea) {
          return Task(
            title: tarea.title,
            type: tarea.type,
            ...
            ), // Reemplaza la fecha límite con el día actual + 1
            pasos: tarea.pasos,
          );
        }).toList();
  ...
  Future<Task> actualizarTarea(int index, Task tareaActualizada) async {
    ...
    final pasos = await generarPasos(
      tareaActualizada.title,
      tareaActualizada.fechaLimite,
    );
  ...
  Future<List<String>> generarPasos(
    String titulo,
