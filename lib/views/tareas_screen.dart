...
import 'package:manazco/views/task_details_screen.dart';
import 'package:manazco/domain/task.dart';
import 'package:manazco/helpers/task_card_helper.dart';
import 'package:manazco/components/add_task_modal.dart'; // Importa el modal reutilizable
...
class TareaScreenState extends State<TareaScreen> {
  ...
  Future<void> _cargarTareasIniciales() async {
    ...
    final tareas = await _tareasService.getTasksWithSteps(
      ...
      builder:
          (context) => AddTaskModal(
            onTaskAdded: (Task nuevaTarea) async {
              Task tareaActualizada = await _tareasService.agregarTarea(
                nuevaTarea,
              ); // El servicio define el tipo
              setState(() {
                _tareas.insert(
                  0,
                  tareaActualizada,
      ...
      builder:
          (context) => AddTaskModal(
            ...
            onTaskAdded: (Task tareaEditada) async {
              Task tareaActualizada = await _tareasService.actualizarTarea(
                index,
                tareaEditada,
              );
              setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:
          _tareas.isEmpty &&
                  !_cargando // Verifica si no hay tareas y no se está cargando
              ? const Center(
                child: Text(
                  ...
                  return GestureDetector(
                    onTap:
                        () => _mostrarDetallesTarea(
                          index,
                        ), // Muestra los detalles al tocar la tarjeta
                    child: Dismissible(
                      key: Key(tarea.title),
                      ...
                      child: construirTarjetaDeportiva(
                        tarea,
                        index,
                        () => _mostrarModalEditarTarea(
                          tarea,
                          index,
                        ), // Pasa la función de edición
                      ),
                      // child: buildTaskCard(
                      //   tarea,
                      //   () {
